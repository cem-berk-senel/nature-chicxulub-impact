...
lo=Ts_l(3,:); hi=Ts_l(2,:);
plot(lo,yy_l,'--g', 'linew', 2.00); hold on;
plot(hi,yy_l,'-g', 'linew', 2.00); hold on;
x2_l = [fliplr(yy_l),yy_l]; inBetween_l = [fliplr(hi),lo];
lo=Ts_o(3,:); hi=Ts_o(2,:);
plot(lo,yy_o,'--','color',[0 150 255]./255,'linew', 2.00); hold on;
plot(hi,yy_o,'-','color',[0 150 255]./255,'linew', 2.00); hold on;
...
print('naturegeo_revision_extfig4.png','-dpng','-r300');